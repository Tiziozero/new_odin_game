...
handle_user_msg :: proc(g: ^Game, buf: ^buffer_io.Buffer, endpoint: net.Endpoint) {
    /*for i in 0..< len(buf.data[:buf.len]) {
    ...
    if msg == networking.MSG_CONNECT {
        ...
        fmt.println("from", id);
        sync.mutex_lock(&g.entities_lock);
        last := g.entities[game.EntityHandle(id)];
...
handle_sender_loop :: proc(g: ^Game, n: int) {
    ...
    for {
        ...
        for k, e in g.entities {
            elapsed := time.diff(e.last_ping, last);
             if elapsed > 2 *time.Second {
                 fmt.println("more than 2 seconds form last ping from user", k);
             } else {
                 fmt.println("sending to", k);
                 // sn, err := net.send_udp(g.socket, buf.data[:n], e);

                 sn, err := net.send_udp(g.socket, buf.data[:buf.len], e.endpoint);
                 if err != .None {
